...
const TestScoresModal = () => {
  const { studentProfile, updateProfile, setCurrentModal, goBack } = useStore();
  const [errors, setErrors] = useState({});

  const fields = ['english_test', 'standardized_test'];
  ...
  return (
    <BaseModal 
      ...
      skipText="Skip Tests" // Add skip option
    >
      <VStack spacing={6}>
        {fields.map(fieldKey => (
          <QuestionField
